<template>
  <a @click="toggleDropdown" @mouseleave="closeDropdown" class="uArJ5e UQuaGc kCyAyd kW31ib foXzLb settings-button" tabindex="0" aria-label="Open Nod extension" role="button">
  ...
  methods: {
    toggleDropdown() {
      if (this.$store.state.settings) {
        ...
        this.$store.dispatch("openDropdown", "settings");
      }
    },
    closeDropdown() {
      this.$store.dispatch("closeDropdown", "settings");
